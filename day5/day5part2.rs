  ...
  for line in input {
    match line.chars().nth(0) {
      ...
      Some('m') => {
        ...
        let mut moving_box:String = String::from("");
        for _i in 1..(move_qty+1){
          match from_stack {
            1 => {moving_box.push(stack_one.pop_back().unwrap());}
            2 => {moving_box.push(stack_two.pop_back().unwrap());}
            3 => {moving_box.push(stack_three.pop_back().unwrap());}
            4 => {moving_box.push(stack_four.pop_back().unwrap());}
            5 => {moving_box.push(stack_five.pop_back().unwrap());}
            6 => {moving_box.push(stack_six.pop_back().unwrap());}
            7 => {moving_box.push(stack_seven.pop_back().unwrap());}
            8 => {moving_box.push(stack_eight.pop_back().unwrap());}
            9 => {moving_box.push(stack_nine.pop_back().unwrap());}
            _ => {}
          }
        }
        for x in moving_box.chars().rev() {
          match to_stack {
            1 => {stack_one.push_back(x);}
            2 => {stack_two.push_back(x);}
            3 => {stack_three.push_back(x);}
            4 => {stack_four.push_back(x);}
            5 => {stack_five.push_back(x);}
            6 => {stack_six.push_back(x);}
            7 => {stack_seven.push_back(x);}
            8 => {stack_eight.push_back(x);}
            9 => {stack_nine.push_back(x);}
            _ => {}
          }
        }
