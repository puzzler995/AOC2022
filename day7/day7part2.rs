...
impl FolderNode {
    ...
    pub fn calc_folder_size(&mut self) {
      ...
      println!("Folder {:?}: {}", self.name, total_size);
    ...
    pub fn get_big_enough_dirs(&mut self, vector:&mut Vec<u32>, needed_size:u32) -> Vec<u32> {
      if self.size.unwrap() >= needed_size {
        vector.push(self.size.unwrap());
      }
      for child in &self.folder_children {
        child.borrow_mut().get_big_enough_dirs(vector, needed_size);
      }
      return Vec::new()
    }
...
fn main() {
  ...
  let mut functional_root = root.borrow_mut();
  functional_root.calc_folder_size();
  let solution = functional_root.get_problem_solution();
  println!("Root Size: {:?}", functional_root.size.unwrap());
  println!("Free Space: {:?}", 70000000-functional_root.size.unwrap());
  println!("Needed Space: {:?}", 30000000-(70000000-functional_root.size.unwrap()));
  let needed_space = 30000000-(70000000-functional_root.size.unwrap());
  let mut big_enough_dirs:Vec<u32> = Vec::new();
  functional_root.get_big_enough_dirs(&mut big_enough_dirs, needed_space);
  big_enough_dirs.sort();
  big_enough_dirs.reverse();
  let delete_solution = big_enough_dirs.pop();
  println!("Delete: {}", delete_solution.unwrap());
  println!("{}", solution);
